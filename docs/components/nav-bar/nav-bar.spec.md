...
9. **Tier-change reactivity:** mounting at Crystalline uses the scroll-driven timeline (no JS scroll listener registered on `window`/scroll container); after `fluidledger:tier-change` to Frosted, the JS scroll-listener fallback is active and the timeline is detached. Reverse transition restores the timeline. No duplicate listeners after repeated transitions.
10. **Frosted uses JS fallback:** at Frosted, a scroll listener drives the shrink (assert listener registered + height responds to scroll events). At Crystalline+, assert NO per-frame JS scroll handler is in the hot path (scroll-driven timeline only).
